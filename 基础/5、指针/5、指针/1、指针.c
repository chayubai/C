...
//电信号转换为数字信号：由1和0组成的二进制序列
//32根地址线产生可能的二进制序列有：
//0000 0000 0000 0000 0000		0
//0000 0000 0000 0000 0001		1
//0000 0000 0000 0000 0010		2
// ...							...
//1111 1111 1111 1111 1111		2^32 - 1
//一共有(2^32 - 1) - (0) + 1 = 2^32个二进制序列，都可以作为内存的编号，把这个编号称为内存单元的地址
...
int main()
{
	...
	//按Fn+ F10调试至return 0；语句后，点击“调试”选项卡，单击“窗口”，选择“内存”，选择任意内存，在地址栏输入&a
	...
	//auto自动变量，在栈上自动创建（申请空间），自动销毁（回收空间）
	//%p是以地址的形式打印
...
//32位机，一个地址（二进制序列）需要32bit的空间，指针变量就需要32bit的空间，即4byte
//64位机，一个地址需要64bit的空间，指针变量就需要64bit的空间，即8byte
*/
...
int main()
{
	...
	struct Stu student = { "张三", 20, 85.5 };//拿定义的结构体类型（学生类型），创建一个变量student
	//结构体的使用：结构体变量名.成员变量名
	//. 结构成员访问操作符：取类型的属性
