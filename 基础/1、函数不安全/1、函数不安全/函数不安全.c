...
//如果显示结果一闪而过怎么解决,针对vs2013，而vs2019不需要设置：
...
/*问题：为什么函数不安全*/
//#define _CRT_SECURE_NO_WARNINGS 1//注释这一行，进行测试
/*
#include <stdio.h>
int main()
{
	char arr[5] = { 0 };//arr数组中只能放5个字符
	scanf("%s",arr);//输入的时候超过5个字符,放入arr数组中
	printf("%s\n",arr);
...
//解决方法：
//1、考虑使用scanf_s()函数，即将scanf函数替换为vs提供的scanf_s函数，注意：scanf_s函数不是C语言提供的，不具备跨平台性，对其他编译器不兼容
...
//4、设置项目属性的预处理：单击项目名称右键，选择“属性”，在“配置属性”中选择“C/C++”，在“C/C++”中选择“预处理器”，在“预处理器定义”中添加“;_CRT_SECURE_NO_WARNINGS”，注意：每创建一个新项目都要设置
