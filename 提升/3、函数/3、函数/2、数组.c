...
//2. strlen是求字符患长度的 - 只能针对字符患求长度–库函数 - 使用得引头文件
...
//	//遍历数组元素
//	int i = 0;
//	for (i = 0;i < (int)strlen(arr); i++)//strlen()默认返回的是一个无符号整型
//	{
//		printf("%c ",arr[i]);
//	}
//	//通常写法
...
//
//	for (i = 0; i < sz; i++)//for (i = 0; i <= sz - 1; i++)
//	{
//		printf("%c ", arr[i]);
//	}
//	return 0;
...
//冒泡排序（升序）的原理：
//每轮相邻两个数比较，前者比后者大，则交换位置，最后每一轮最后一个数都会是最大的数。
...
//	return 0;
//}
//如果给的数组数字为9，1，2，3，4，5，6，7，8，则上面的程序效率较低
//改进：当里面有一轮两两比较发现，都不需要交换，则判断为已经有序
//#include <stdio.h>
...
//9. 数组的应用实例1：三子棋
//见项目"3、函数"：gameTest.c、game.c、game.h
//10. 数组的应用实例2：扫雷游戏
//见项目"game"：gameTest.c、game.c、game.h