...
//	return 0;
//}
//调试-窗口-内存，输入&a得到44 33 22 11（地址的存放顺序可能顺的可能倒的）
//当调试到*pa = 0;&a地址的内容变为了00 00 00 00
...
//}
//调试-窗口-内存，输入&a得到44 33 22 11（地址的存放顺序可能顺的可能倒的）
//当调试到*pa = 0;&a地址的内容变为了00 33 22 11
...
//   int* p =  test();//接收到了a的地址，
//   printf("%d\n",*p);//非法访问
//   *p = 20;//此时访问的是别人的空间，不在是a的空间，但地址相同
//    return 0;
//}
//这种是正确的
//int* test()
//{
//    static int a = 10;//局部变量出作用域，内存被销毁
//    return &a;
//}
//int main()
//{
...
//		如：vp可以和数组最后一个元素的越界的地址&arr[-1]比较，但不可以与第一个元素的越界的地址&arr[5]比较*/
//	return 0;
//}
...
//}
//注意：内存的空间大小，是按字节编址的，
//但是存放的数据类型不同所占用的空间不同，此时相邻元素的地址就不一定是相邻的，可能差某个倍数关系
//如：int arr[5];假设&0arr[0] = 0x11223300
//则&0arr[1] = 0x11223304，&0arr[2] = 0x11223308，&0arr[3] = 0x1122330c
//如：char arr[5];假设&0arr[0] = 0x11223300
//则&0arr[1] = 0x11223301，&0arr[2] = 0x11223302，&0arr[3] = 0x11223303
...
//整型数组-存放的是整型
//字符数组-存放的是字符
//指针数组-存放的是同一类指针类型的地址，如：int* p[10];char* p[10];...
//数组指针-存放数组地址的指针，如：int arr[10];int* p = arr;
//int main()
//{
//	int a = 10;
//	int b = 20;
