...
//调试相关的知识
//创建一个源文件，当为Debug时，编译后，存放代码的文件夹中会出现Debug文件夹，里面包含了源文件编译好的Debug版本的可执行文件，（
//前提：代码加#include <stdlib.h>和system("pause")）点击可以运行

//创建一个源文件，当为Release时，编译后，存放代码的文件夹中会出现Release文件夹，里面包含了源文件编译好的Release版本的可执行文件，（
//前提：代码加#include <stdlib.h>和system("pause")）点击可以运行

//观察发现二者的可执行文件大小不一样，Debug编译的可执行文件较大
//此版本的源程序可调试，因为文件中包含了调试的相关信息
...
//2、最常使用的几个快捷键：
	...
	//再按F5，发现执行直接来到断点标记的位置，且程序打印了前面的执行结果
...
//缺点：不能任意添加想展示的内容，随着程序的进行部分展示的内容会消失
//调试-窗口-监视：满足了以上缺点
...
//}
//通过F10/F11搭配使用，观察不同阶段的变化
//发现每进入一个函数，栈内有底向上加一个信息，每退出一个函数，栈内有顶向下减少一个信息
...
//7、修改代码ret = 1放在int i = 0处。ctrl+F5运行
...
//再往后调试，此时就会非法访问，当i=10,11,12，观察arr[10]、arr[11]、arr[12]被改为了0，当arr[12]==0时，i==0了，故错误了
//为什么arr[12]赋值为0的同时，i也改变为0了？通过观察&arr[12]和&i，发现它们的地址相同。
//为什么它们是同一块空间？
//分析：
//局部变量存放在栈区中
//栈区的默认使用：先使用高地址处的空间，再使用低地址处的空间
//数组随着下标的增长，每个元素的地址是由低到高变化的
//假设高地址0xFF120A0C，开辟了i变量的空间，arr数组每个元素开辟的空间从0xFF120A00-0xFF120A09
//当i++到10时，数组越界访问，0xFF120A0A、0xFF120A0B、0xFF120A0C地址的空间，并将其内容改为0

//由于程序忙着死循环，故不会报错
//对于不同的机器内存布局，死循环的条件不同
//将int i = 0;放在数组定义的后面，就不会出现死循环，但会报错
//将int i = 0;放在for()中定义，C语言语法不支持这种书写，很多编译器是无法编译通过，此为C++的定义
...
//应该对两个版本进行测试，以确保Debug和Release版本下都没有问题
//从事测试的人员，应该记录测试出错结果的证据，防止下次没有出现
...
//	char arr1[] = "###########";
//	char arr2[] = "bit";
//	strcpy(arr1, arr2);//将arr2的内容拷贝到arr1中，包括'\0'
//	printf("%s\n", arr1);
//	return 0;
//}
//模拟实现strcpy
...
//void my_strcpy(char* destination, char* scoure)
//{
//	//防止传入空指针，导致程序崩溃，引起其他程序出错，所以假设if判断
...
//		;
//	return ret;//不是直接返回destination的地址，返回的是起始地址
//}
//int main()
//{
//	char arr1[] = "###########";
