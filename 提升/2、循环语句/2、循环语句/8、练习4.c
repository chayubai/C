...
int main()
{
	...
	a = a - b;//第三步：a - b，a的内容没有发生变化，b的内容发生变化，和减去b此时得到了原来的b，放入a中，此时a得到了原来的b
	printf("交换后: a=%d b=%d\n", a, b);
...
//加减法：只能解决两个较小数字的交换，即两个数的和不能超过INT_MAX的值
...
//方法二：给定数组中，只能找一个单数，且每个数只能重复出现偶数次，不能出现奇数次，比如3重复出现了3次
/*
#include <stdio.h>
int main()
{
