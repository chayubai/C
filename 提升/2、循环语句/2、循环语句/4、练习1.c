...
//ret = 1*1=1
//ret = 1*1*2=2
//ret = 2*1*2*3=12      这里应该ret = 1 * 1 * 2 * 3
//ret会一直累加，因此ret每次求阶乘都需要从1开始累乘。
...
int main()
{
	...
	{//要循环二分查找的动作
		mid = (left + right) / 2;//通过元素个数/2找到中间元素是不可取的
		if (arr[mid] < key)
		{
			left = mid + 1;		
		}
...
int main()
{
	...
	printf("arr1[] = abc的大小为：%d\n", sizeof(arr3));//4
	printf("arr3[] = abc的元素个数为：%d\n", sizeof(arr3) / sizeof(arr3[0]));//4
	printf("arr3[] = abc的长度为：%d\n", strlen(arr3));//3
	//针对此方法初始化的字符串数组：
	//数组的大小：一个元素的大小 * 元素个数 = 1byte * 4 = 4byte
	//求数组元素个数：计算方法：sizeof(arr3) / sizeof(arr3[0])，包含计算了'\0'
	//求数组长度：计算方法：strlen(arr3)，不计算'\0'
	//数组中最后一个元素的下标(非\0)：计算方法：sizeof(arr3) / sizeof(arr3[0]) - 2或者strlen(arr3) - 1
	return 0;
}
*/
//注意：strlen求的是字符串的长度，而不是整型数组的长度
...
int main()
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        //if(paaword == "123456")//此方法比较的是两个字符串的首字符的地址。==不能用来比较两个字符串是否相等，应该使用strcmp--string.h
        ...
        {   //MSDN搜索strcmp
			...
			//如：abcdef和abccqqqqq
			//前面两字符各自的abc字符ASCII码值相等，则比较d和c，d的ASCII码值大于c，则abcdef字符串大
            printf("登录成功\n");
            break;
        }
        else
...
*/
//优化bug：当输入“1234 56”时，会出错，第一次scanf读取1234，第二次scanf读取56